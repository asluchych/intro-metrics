...
#                                                                              #
################################################################################
# necessary packages: if not installed, type install.packages(c('lmtest', 'sandwich', 'nlme', 'orcutt')) 
# in the console and press Enter
library(lmtest) # for function: bgtest, NeweyWest
...
# c) Breusch-Godfrey Test for First-Order Autocorrelation
# save residuals to use as dependent variable in auxiliary regression
vacancies$resid <- regA$residuals
# save lagged residuals to use as explanatory variable in auxiliary regression
vacancies$resid_lag <- c(0, regA$residuals[1:nrow(vacancies)-1]) # as default in EViews: 
...
# 95% confidence interval for beta_2
# get coefficients
co_hac <- coeftest(regA, vcov = NeweyWest(regA, lag = 3, prewhite = FALSE, adjust = TRUE)) # as default in EViews
co_hac
# calculate upper and lower bound 
...
# e) # assumption: errors follow AR(1) error model, Generalized Least Squares Estimation 
...
# Is the estimate for ρ significantly different from zero at a 5% significance level?
# regE <- gls(log(jv) ~ log(u), correlation = corAR1(), data = vacancies)
# summary(regE) 